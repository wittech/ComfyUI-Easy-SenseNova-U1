...
# SenseNova-U1 is a PyTorch-only runtime. Do not let an unrelated TensorFlow
# installation become an implicit dependency of this private Transformers copy.
USE_TF = "0"
...
def direct_transformers_import(path: str, file="__init__.py") -> ModuleType:
    ...
    """
    # This source snapshot is intentionally imported as ``transformers_4571``.
    # Reuse that already initialized private root instead of executing it again
    # under the global ``transformers`` name.
...
def clear_import_cache() -> None:
    ...
    transformers_modules = [mod_name for mod_name in sys.modules if mod_name.startswith("transformers_4571.")]
    ...
    if "transformers_4571" in sys.modules:
        main_module = sys.modules["transformers_4571"]
